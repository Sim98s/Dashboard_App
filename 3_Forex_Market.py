...
with st.sidebar:
    ...
    with col_left:
        for pair_code, pair_name in all_pairs_items[:half]:
            ...
            if st.checkbox(pair_name, value=is_selected, key=f"multi_{pair_code}"):
                if pair_code not in st.session_state.multi_pairs and len(st.session_state.multi_pairs) < 20:
    ...
    # Section 2 : Paire principale - LISTE DÉROULANTE AMÉLIORÉE
    st.markdown('<div class="sidebar-card">', unsafe_allow_html=True)
    st.markdown('<div class="sidebar-title"> Main Pair Analysis</div>', unsafe_allow_html=True)
    
    # Préparer les options pour le selectbox
    pair_options_names = []  # Pour l'affichage (noms seulement)
    pair_options_codes = []  # Pour les valeurs (codes)
    
    for code, name in FOREX_PAIRS.items():
        pair_options_names.append(name)  # Ex: "EUR/USD"
        pair_options_codes.append(code)  # Ex: "EURUSD=X"
    
    # Trouver l'index actuel
    current_pair_name = FOREX_PAIRS[st.session_state.selected_pair]
    selected_index = pair_options_names.index(current_pair_name)
    
    # Sélecteur avec noms seulement
    ...
    selected_pair_display = st.selectbox(
        "Choose a pair:",
        options=pair_options_names,
        index=selected_index,
        label_visibility="collapsed"
    )
    
    # Mettre à jour le code correspondant
    selected_index = pair_options_names.index(selected_pair_display)
    selected_pair_code = pair_options_codes[selected_index]
    st.session_state.selected_pair = selected_pair_code
    
    # Afficher la paire sélectionnée avec le code technique en plus petit
...
with tab2:
    # Section 2: Technical analysis of main pair - CORRIGÉ: seulement le nom de la paire
    selected_pair_name = FOREX_PAIRS.get(st.session_state.selected_pair, st.session_state.selected_pair)
    
    # En-tête amélioré pour Technical Analysis
    st.markdown(f"""
    <div style="background: linear-gradient(135deg, var(--bg-card) 0%, #2D3748 100%); border-radius: 12px; padding: 1.5rem; margin-bottom: 2rem; border: 1px solid var(--border-color); box-shadow: var(--shadow);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                <h2 style="color: var(--accent-color); margin-bottom: 0.3rem; font-size: 2rem;">{selected_pair_name}</h2>
                <div style="color: var(--text-secondary); font-size: 0.95rem; display: flex; align-items: center; gap: 1rem;">
                    <span> Technical Analysis</span>
                    <span style="color: var(--accent-color); font-weight: 600;">•</span>
                    <span>Timeframe: <strong>{st.session_state.timeframe.upper()}</strong></span>
                </div>
            </div>
            <div style="text-align: right;">
                <div style="display: inline-block; background: rgba(96, 165, 250, 0.1); padding: 0.5rem 1rem; border-radius: 8px; border: 1px solid var(--accent-color);">
                    <div style="color: var(--text-secondary); font-size: 0.8rem; margin-bottom: 0.2rem;">Active Indicators</div>
                    <div style="color: var(--accent-color); font-weight: 700; font-size: 1.1rem;">
                        {sum([st.session_state.show_sma, st.session_state.show_ema, st.session_state.show_rsi, 
                              st.session_state.show_macd, st.session_state.show_bollinger])}/5
                    </div>
                </div>
            </div>
        </div>
    ...
    if not data.empty and len(data) > 10:
        ...
        # Main chart - TITRE CORRIGÉ
        st.markdown(f"### {selected_pair_name} - Price Chart & Indicators")
        ...
        fig = make_subplots(
            ...
            row_heights=[0.7, 0.3],
            subplot_titles=(f'{selected_pair_name} - Price Action', 'Volume Analysis')
        )
        ...
        if st.session_state.show_rsi or st.session_state.show_macd:
            ...
            indicator_fig = make_subplots(
                ...
                subplot_titles=('RSI (14) - Momentum Indicator', 'MACD - Trend Indicator'),
                row_heights=[0.5, 0.5]
            )
...
<div style="text-align: center; color: var(--text-secondary); font-size: 0.9rem; padding: 1.5rem;">
    <p>Forex Market Analytics • Multi-pairs • Technical Analysis • Correlation Analysis</p>
    <p style="font-size: 0.8rem; margin-top: 0.5rem; opacity: 0.7;">
        Version 3.6 • Forex Market Analytics • 20 Pairs Limit • API/Real-Time Data
    </p>
</div>
""", unsafe_allow_html=True)